...
import requests
OUT=Path('artifacts/kira_euro19_opponent_tt_u35_common_2025');OUT.mkdir(parents=True,exist_ok=True)
START=date(2024,12,27);END=date(2025,12,17);SEASONS=['2425','2526'];LEAGUES=['E0','E1','E2','E3','SP1','SP2','D1','D2','I1','I2','F1','F2','N1','P1','SC0','SC1','SC2','SC3','B1'];REQ=['Date','HomeTeam','AwayTeam','AvgCH','AvgCD','AvgCA'];P=.50;MAXD=3
...
def main():
    s=requests.Session();s.headers['User-Agent']='KIRA-EURO19-OPPTTU35-COMMON25/1.0 outcome-blind';eligible=[];audit=[];seen=set()
    for season in SEASONS:
      for lg in LEAGUES:
        ...
        for row in rows[hi+1:]:
            ...
            seen.add(key);sel=home if side=='HOME' else away;opp=away if side=='HOME' else home;price=h if side=='HOME' else a;eid='E19C-'+hashlib.sha256('|'.join(key).encode()).hexdigest()[:20]
    ...
    write_csv(OUT/'euro19_opponent_tt_u35_rows_common_2025.csv',sel);cnt=Counter(x['date'] for x in sel)
    r={'hypothesis_id':'FOOTBALL_EURO19_OPPONENT_TT_U35_BROAD_V1','mode':'OUTCOME_BLIND_COMMON_WINDOW_RECONSTRUCTION','window':[START.isoformat(),END.isoformat()],'calendar_days':(END-START).days+1,'selected_legs':len(sel),'eligible_pre_cap':len(eligible),'candidate_dates':len(cnt),'date_leg_count_distribution':dict(sorted(Counter(cnt.values()).items())),'three_leg_dates':sum(v>=3 for v in cnt.values()),'outcomes_loaded':False,'outcome_columns_requested':False,'source_audit':audit};(OUT/'summary.json').write_text(json.dumps(r,indent=2));print(json.dumps(r,indent=2))
