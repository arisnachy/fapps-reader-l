...
import kira_summer16_opponent_tt_u35_broad_v1 as base

OUT=Path('artifacts/kira_summer16_opponent_tt_u35_margin_p50_55_v1');OUT.mkdir(parents=True,exist_ok=True)
BLOCKS=[('DEV_2015',2015),('OOS_2016',2016)];LOW=.50;HIGH=.55;MAXD=3
...
def score(label,year):
    bd=OUT/label.lower();bd.mkdir(parents=True,exist_ok=True);pre,out,audit,dups=base.fetch_year(year)
    source_ok=len(audit)==len(base.CODES) and all(x.get('status')=='PASS' for x in audit)
    marginal=[x for x in pre if LOW<=float(x['p_selected_novig'])<HIGH]
    if not source_ok or dups:
        r={'block':label,'year':year,'status':'SOURCE_GATE_FAIL','source_audit':audit,'duplicate_event_keys':dups};(bd/'summary.json').write_text(json.dumps(r,indent=2));return r
    by=defaultdict(list)
