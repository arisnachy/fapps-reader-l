...
import requests
OUT=Path('artifacts/kira_extra16_favorite_plus1_5_multi3_tierb_p055_2019');OUT.mkdir(parents=True,exist_ok=True)
CODES=['ARG','AUT','BRA','CHN','DNK','FIN','IRL','JPN','MEX','NOR','POL','ROU','RUS','SWE','SWZ','USA']
REQ=['Date','Home','Away','League','AvgCH','AvgCD','AvgCA','HG','AG']
YEAR=2019;TH=.55;MAX3=3
MIN_LEGS=400;MIN_LEG_RATE=.95;MIN_LEG_LCB=.93
MIN_DATES=200;MIN_BUNDLE_RATE=.92;MIN_BUNDLE_LCB=.90
...
def main():
    s=requests.Session();s.headers['User-Agent']='KIRA-X16-P055-TIERB2019/1.0'
    ...
    for code in CODES:
        ...
        for row in rows[1:]:
            ...
            seen.add(key);eid='X19B-'+hashlib.sha256('|'.join(key).encode()).hexdigest()[:20]
            pre.append({'date':d.isoformat(),'source':code,'league':lg,'Home':home,'Away':away,'selected_side':side,'selected_entity':ent,'selected_price':price,'p_favorite_novig':prob,'event_id':eid})
    ...
    nl=len(settled);wl=sum(x['hit'] for x in settled);nd=len(bundles);wd=sum(x['survived'] for x in bundles);lr=wl/nl if nl else 0;br=wd/nd if nd else 0;ll,_=wilson(wl,nl);bl,_=wilson(wd,nd)
    ...
    res={'hypothesis_id':'FOOTBALL_EXTRA16_FAVORITE_PLUS1_5_MULTI3_TIERB_P055_V1_2019','threshold':TH,'selected_legs':nl,'leg_wins':wl,'leg_losses':nl-wl,'leg_rate':lr,'leg_wilson95_lcb':ll,'candidate_dates':nd,'bundle_wins':wd,'bundle_losses':nd-wd,'bundle_rate':br,'bundle_wilson95_lcb':bl,'date_leg_count_distribution':dict(sorted(dist.items())),'source_selected_counts':dict(sorted(src.items())),'failure_source_counts':dict(sorted(failsrc.items())),'max_team':team.most_common(1)[0] if team else None,'max_month':month.most_common(1)[0] if month else None,'source_audit':audit,'selected_ledger_sha256':ledger_sha,'duplicate_event_keys':dups,'candidate_generation_used_outcomes':False,'leg_gate_pass':nl>=MIN_LEGS and lr>=MIN_LEG_RATE and ll>=MIN_LEG_LCB,'bundle_gate_pass':nd>=MIN_DATES and br>=MIN_BUNDLE_RATE and bl>=MIN_BUNDLE_LCB}
    res['decision']='TIERB_COVERAGE_PASS' if res['leg_gate_pass'] and res['bundle_gate_pass'] else 'NO_PASS'
