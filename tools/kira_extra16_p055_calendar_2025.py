...
import requests
OUT=Path('artifacts/kira_extra16_p055_calendar_2025');OUT.mkdir(parents=True,exist_ok=True)
CODES=['ARG','AUT','BRA','CHN','DNK','FIN','IRL','JPN','MEX','NOR','POL','ROU','RUS','SWE','SWZ','USA']
REQ=['Date','Home','Away','League','AvgCH','AvgCD','AvgCA']
START=date(2024,12,27);END=date(2025,12,17);TH=.55
...
def main():
    s=requests.Session();s.headers['User-Agent']='KIRA-X16-P055-CALENDAR25/1.0';audit=[];eligible=[];seen=set()
    for code in CODES:
        ...
        for row in rows[1:]:
            ...
            seen.add(key);eid='X55-'+hashlib.sha256('|'.join(key).encode()).hexdigest()[:20]
