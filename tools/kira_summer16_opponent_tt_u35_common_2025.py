...
import requests
OUT=Path('artifacts/kira_summer16_opponent_tt_u35_common_2025');OUT.mkdir(parents=True,exist_ok=True)
CODES=['ARG','AUT','BRA','CHN','DNK','FIN','IRL','JPN','MEX','NOR','POL','ROU','RUS','SWE','SWZ','USA']
REQ=['Date','Home','Away','League','AvgCH','AvgCD','AvgCA'];START=date(2024,12,27);END=date(2025,12,17);P=.50;MAXD=3
...
def main():
    s=requests.Session();s.headers['User-Agent']='KIRA-SUMMER16-U35-COMMON25/1.0 outcome-blind';eligible=[];audit=[];seen=set()
    for code in CODES:
        ...
        for row in rows[1:]:
            ...
            seen.add(key);sel=home if side=='HOME' else away;opp=away if side=='HOME' else home;price=h1 if side=='HOME' else a1;eid='S16C-'+hashlib.sha256('|'.join(key).encode()).hexdigest()[:20]
