...
OUT=Path('artifacts/kira_euro19_opponent_tt_u35_margin_p45_50_v1');OUT.mkdir(parents=True,exist_ok=True)
LEAGUES=['E0','E1','E2','E3','SP1','SP2','D1','D2','I1','I2','F1','F2','N1','P1','SC0','SC1','SC2','SC3','B1']
BLOCKS=[('DEV_1819','1819'),('OOS_1920','1920')]
REQ=['Date','HomeTeam','AwayTeam','AvgCH','AvgCD','AvgCA','FTHG','FTAG'];LOW=.45;HIGH=.50;MAXD=3
...
def write(path,rows):
    ...
    with path.open('w',newline='',encoding='utf-8') as fh:
        w=csv.DictWriter(fh,fieldnames=fs);w.writeheader();w.writerows(rows)
def fetch(season):
    s=requests.Session();s.headers['User-Agent']='KIRA-EURO19-U35-P45-50/1.0';pre=[];out={};audit=[];seen=set()
    for lg in LEAGUES:
        ...
        ix={c:header.index(c) for c in REQ};mx=max(ix.values());eligible=0
        for row in rows[hi+1:]:
            ...
            seen.add(key);sel=home if side=='HOME' else away;opp=away if side=='HOME' else home;price=h if side=='HOME' else a;eid='E19M45-'+hashlib.sha256('|'.join(key).encode()).hexdigest()[:20]
            pre.append({'date':d.isoformat(),'season':season,'league':lg,'Home':home,'Away':away,'selected_side':side,'selected_entity':sel,'opponent_entity':opp,'selected_price':price,'p_selected_novig':prob,'event_id':eid});out[eid]={'home_goals':hg,'away_goals':ag};eligible+=1
        audit.append({'season':season,'league':lg,'status':'PASS','sha256':sha,'encoding':enc,'eligible_band_pre_cap':eligible})
