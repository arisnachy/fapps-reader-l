   ...
   for i in range(loc.count()):
    try:
     ...
     if n.is_visible():
      ...
      n.click(force=True,timeout=6000);clicks.append(meta);p.wait_for_timeout(6000);raise StopIteration
    except StopIteration: raise
    except Exception: pass
  res['soccer_clicks']=clicks
  res['body_text']=clean(p.locator('body').inner_text(timeout=30000))[:180000]
  # capture every short visible leaf text plus nearest clickable ancestor; never invoke it
...
if __name__=='__main__':
 try: main()
  ...
  # Never expected outside click loop; kept to avoid masking output in browsers with odd locator behavior.
  main()
